...
import { Counter, CounterMap, LocationMap, Polygon, Stack, StackMap } from '../../types/game-types';

const updateCanvas = (canvas: HTMLCanvasElement, scale: number, boardName: string, stacks: StackMap, counters: CounterMap, locationMap?: LocationMap) => {
...
export const Map = (props: PropTypes) => {
    ...
    React.useEffect(() => {
        ...
        if (canvasRef.current && boardImageName !== undefined) {
            ...
            updateCanvas(canvas, scale, boardImageName, stacks, counters, locationMap);
        }
    }, [canvasRef, scale, boardImageName, stacks, counters, locationMap]);
